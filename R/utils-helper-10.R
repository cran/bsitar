  ...
  if(dist != 'flat') {
    ...
    for (i in 1:length(x)) {
      ...
      if (pname_ %in% set_str_names) {
        ...
        method_location_scale <- "original" 
        ...
        if (grepl("^location$", pname_)) {
          ...
          # location a b c d e random effects (class sd, typically 0)
        ...
        if (grepl("^scale$", pname_)) {
          # scale a b c d e f g h i fixed effects (class b)
          if (nlpar == "a" & class == "b" & grepl("a", fixedsi)) {
          ...
          #   lm2 -> lmse / sd(Xmt)
          #   lm3 -> lmse
          # where lmse is the standard error from the lm fit.
          if (nlpar == "s" & class == "b") {
            if (x_i == paste0("lm", empty_sufx)) {
              if (s_form_0) {
          ...
          # scale a b c d e f g h i random effects (class sd)
          if (nlpar == "a" & class == "sd" & grepl("a", randomsi)) {
            if (x_i == paste0("ysd", empty_sufx)) {
              eit <-  gsub("ysd", paste0("ysd", resp_), x_i)
              evaluated_parameter <- scale_factor * ept(eit)
