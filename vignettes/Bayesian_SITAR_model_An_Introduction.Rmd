...
\zeta_0   & \sim \operatorname{normal}(\ 0,\ 2.0) \\
\gamma_0  & \sim \operatorname{normal}(\ 0,\ 1.0) \\
\beta_1 \text{,..,} \beta_r & \sim \operatorname{normal}(\ \mathbf{\beta_{lm}}, \ \mathbf {X_{Spl}}) \\
\alpha_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {y_{sd}}) \\
\zeta_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {2.0}) \\
\gamma_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {1.0}) \\
...
diagonal of the matrix. The assumption of homoscedasticity (constant variance) of the residuals can be relaxed. A detailed description of the prior distributions for each parameter is provided in below Section [priors].
...
[rstanrm](https://rdrr.io/cran/rstanarm/man/priors.html) packages. The default 
setting for `autoscale` in 'bsitar' package is `FALSE` i.e., the scale factor is
set to `1`.
...
\newpage
## Initials
We use the default initial values recommended by the author of the 'bsitar'
package. Under these default settings, user-specified initial values are
assigned to the fixed-effect growth parameters (a, b, c, d) and the spline
coefficients, while random initial values are assigned to the
variance–covariance parameters, drawn uniformly from [−2,2] on the unconstrained
scale. For the fixed-effect parameter a and the spline coefficients, initial
values are obtained by fitting a simple linear model to the data; for the
remaining fixed-effect parameters (b, c and d), the initial values are set to
zero. Note that the sitar package uses a similar approach for setting initial
values for the fixed-effect parameters. A similar approach is used for the
covariate-specific growth parameters (a, b, c, and d): initial values for
parameter a are obtained from a simple linear model fit to the data, while the
remaining fixed-effect parameters (b, c and d) are assigned an initial value of
zero. Note that the 'bsitar' package allows full control in specifying custom
initial values for each of the fixed effects as well as the random effects
(variance covariance parameters as well as the residual variance) via dedicated
argument.
...
\zeta_0^\text{Y1} & \sim \operatorname{normal}(\ 0, 2.0) \\
\zeta_0^\text{Y2} & \sim \operatorname{normal}(\ 0, 2.0) \\
\gamma_0^\text{Y1} & \sim \operatorname{normal}(\ 0, 1.0) \\
\gamma_0^\text{Y2} & \sim \operatorname{normal}(\ 0, 1.0) \\
\beta_1^\text{Y1} \text{,..,} \beta_r^\text{Y1} & \sim \operatorname{normal}(\ \mathbf {\beta^\text{Y1}_{lm}}, \ \mathbf {X^\text{Y1}_{Spl}}) \\
\beta_1^\text{Y2} \text{,..,} \beta_r^\text{Y2} & \sim \operatorname{normal}(\ \mathbf {\beta^\text{Y2}_{lm}}, \ \mathbf {X^\text{Y2}_{Spl}}) \\
\alpha_j^\text{Y1}   & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y1}_{sd}}) \\
\alpha_j^\text{Y2}   & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y2}_{sd}}) \\
\zeta_j^\text{Y1}  & \sim \operatorname{normal_{Half}}(\ {0},\ {2.0}) \\
\zeta_j^\text{Y2} & \sim \operatorname{normal_{Half}}(\ {0},\ {2.0}) \\
\gamma_j^\text{Y1} & \sim \operatorname{normal_{Half}}(\ {0},\ {1.0}) \\
\gamma_j^\text{Y2} & \sim \operatorname{normal_{Half}}(\ {0},\ {1.0}) \\
